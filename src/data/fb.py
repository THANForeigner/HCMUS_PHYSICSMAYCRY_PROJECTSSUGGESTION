...
def firebase_register(email, pwd, name):
    """Đăng ký, cập nhật tên hiển thị và tạo Profile mặc định trong DB"""
    # 1. Đăng ký tài khoản
    ...
    try:
        ...
        # 2. Cập nhật Display Name trong Auth
        id_token = auth_data["idToken"]
        update_url = f"{TOOLKIT_URL}update?key={FIREBASE_API_KEY}"
        requests.post(update_url, json={"idToken": id_token, "displayName": name})

        # 3. Tạo dữ liệu người dùng trong Realtime Database
        # Gọi hàm tạo profile bên dưới để đồng bộ
        auth_data["displayName"] = name  # Gán tạm để hàm tạo profile dùng
...
def create_new_user_profile(user_auth_data, user_name):
    """
    Tạo entry mới trong node 'users/{uid}'
    """
...
def firebase_register(email, pwd, name):
    ...
    try:
        ...
        requests.post(f"{TOOLKIT_URL}update?key={FIREBASE_API_KEY}",
                      json={"idToken": id_token, "displayName": name})

        # Lưu user profile mặc định (Tuỳ chọn, có thể bỏ qua nếu chưa cần)
        return data
    except Exception as e:
        return {"error": str(e)}
...
def _parse_firestore_value(field_value):
    if 'stringValue' in field_value:
        ...
        return [_parse_firestore_value(v) for v in field_value['arrayValue'].get('values', [])]
